...
inline bool Frustum::IsBoxVisible(const dmVMath::Vector3& minp, const dmVMath::Vector3& maxp) const
{
    ...
    {
        const dmVMath::Vector4 plane = m_Planes[i];
        if ((dot(plane, Vector4(minp.getX(), minp.getY(), minp.getZ(), 1.0f)) < 0.0) &&
            (dot(plane, Vector4(maxp.getX(), minp.getY(), minp.getZ(), 1.0f)) < 0.0) &&
            (dot(plane, Vector4(minp.getX(), maxp.getY(), minp.getZ(), 1.0f)) < 0.0) &&
            (dot(plane, Vector4(maxp.getX(), maxp.getY(), minp.getZ(), 1.0f)) < 0.0) &&
            (dot(plane, Vector4(minp.getX(), minp.getY(), maxp.getZ(), 1.0f)) < 0.0) &&
            (dot(plane, Vector4(maxp.getX(), minp.getY(), maxp.getZ(), 1.0f)) < 0.0) &&
            (dot(plane, Vector4(minp.getX(), maxp.getY(), maxp.getZ(), 1.0f)) < 0.0) &&
            (dot(plane, Vector4(maxp.getX(), maxp.getY(), maxp.getZ(), 1.0f)) < 0.0))
        {
            return false;
        }
    }
