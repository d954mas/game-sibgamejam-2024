...
inline void LightSetBrightness(Light* light, float brightness) {
    if (light->color.getW() != brightness) {
        light->color.setW(brightness);
        light->dirty = true;
    }
}
...
inline void LightWriteToBuffer(Light* light, uint8_t* values,  uint32_t stride) {
    ...
    int radius_full = static_cast<int>(light->radius);
    float radius_fract = light->radius - radius_full;
    ...
    values[0] = radius_full/ LIGHT_RADIUS_MAX * 255;
...
static int LuaLightSetBrightness(lua_State* L) {
    DM_LUA_STACK_CHECK(L, 0);
    check_arg_count(L, 2);
    LuaLightUserData* userData = LightUserdataCheck(L, 1);
    LightSetBrightness(userData->light, luaL_checknumber(L, 2));

    return 0;
}

static int LuaLightGetBrightness(lua_State* L) {
    DM_LUA_STACK_CHECK(L, 0);
    check_arg_count(L, 2);
    LuaLightUserData* userData = LightUserdataCheck(L, 1);
    lua_pushnumber(L,userData->light->color.getW());
...
static const luaL_Reg functions[] = {
    ...
    {"set_brightness", LuaLightSetBrightness},
    {"get_brightness", LuaLightGetBrightness},
...
static int LuaLightsManagerDestroyLight(lua_State* L){
    ...
    //remove light from lights in world
    for (int i = 0; i < g_lightsManager.lightsInWorld.Size(); ++i) {
        if(g_lightsManager.lightsInWorld[i]==userData->light){
            g_lightsManager.lightsInWorld.EraseSwap(i);
            break;
        }
    }
